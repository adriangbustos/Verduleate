...
export class SignUpComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private authService: AuthService, private router: Router, private messageService: MessageService, private cdRef: ChangeDetectorRef) {
  ...
  async submit() {
    if (this.signUpForm.valid) {
      ...
      try {
        ...
        await this.authService.createDocument('users', user.uid, {
          email: user.email,
          uid: user.uid,
          onboarding: false,
  ...
  private handleAuthError(errorCode: string) {
    switch (errorCode) {
      case 'auth/email-already-in-use':
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Este correo ya está en uso. Intenta con otro. Sugerimos usar un mail personal.' });
        break;
      default:
